...
public class MusicTable extends Application {
	...
	ObservableList<String> artists = FXCollections.observableArrayList("Adele",
			"Unknown", "Beyounce", "Rihanna", "Avril", "Disturbia", "Kid Rock",
			"Jessi J", "Unknown", "Unknown");
	ObservableList<String> titles = FXCollections.observableArrayList(
			"Running in the Deep", "Title 01", "Title 09", "What's my name",
			"What the Hell", "Disturbia", "Kid Rock", "Price Tag", "Title 2",
			"09");
	...
	public TableView getTableView() {
		...
		/*
		 * Creating the TableColumn for the TableView The property value Factory
		 * name must match with the Generic Class's(Music's) property
		 */
		TableColumn<Music, Album> albumArt = new TableColumn<Music, Album>(
				"Album Art");
		albumArt.setCellValueFactory(new PropertyValueFactory("album"));
		albumArt.setPrefWidth(200);

		TableColumn<Music, String> title = new TableColumn<Music, String>(
				"Title");
		title.setCellValueFactory(new PropertyValueFactory("title"));
		title.setPrefWidth(120);

		TableColumn<Music, String> artist = new TableColumn<Music, String>(
				"Artist");
		artist.setCellValueFactory(new PropertyValueFactory("artist"));
		artist.setPrefWidth(120);

		TableColumn<Music, Integer> rating = new TableColumn<Music, Integer>(
				"Rating");
				...
				TableCell<Music, Album> cell = new TableCell<Music, Album>() {
					...
					public void updateItem(Album item, boolean empty) {
						if (item != null) {
							...
							imageview.setImage(new Image(MusicTable.class
									.getResource("img").toString()
									+ "/"
									+ item.getFilename()));
		...
		rating.setCellFactory(new Callback<TableColumn<Music, Integer>, TableCell<Music, Integer>>() {
			@Override
			public TableCell<Music, Integer> call(
					TableColumn<Music, Integer> param) {
	...
	public Group getBackground() {
		...
		rect.setEffect(DropShadowBuilder.create().color(Color.web("#969696"))
				.offsetX(0).offsetY(0).radius(50).spread(0.2).build());
